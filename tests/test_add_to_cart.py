...
class TestAddProductToCart(GeneralBaseClass):
    def test_add_single_product_to_car(self):
        ...
        is_exists = cart.is_product_in_cart(PRODUCTS["backpack"])
        cart.return_to_products_list()
        assert is_exists
    ...
    def test_add_multiple_products_to_cart(self, product_name):
        ...
        products_page.open_cart()
        cart = CartPage(self.driver)
        assert cart.is_product_in_cart(PRODUCTS[product_name]), f"{PRODUCTS[product_name]} is not available in the cart"
        cart.return_to_products_list()
